...
db_exists = os.path.exists('review_tree.db')
...
def initialize_database():
    ...
    if cursor.fetchone()[0] == 0:
        ...
        cursor.executemany('INSERT OR IGNORE INTO Phrases (phrase) VALUES (?)', [(phrase,) for phrase in phrases])

        # Populate the PhraseRelations table
        for parent, children in review_tree.items():
            parent_id = cursor.execute('SELECT id FROM Phrases WHERE phrase = ?', (parent,)).fetchone()[0]
            for child in children:
                child_id = cursor.execute('SELECT id FROM Phrases WHERE phrase = ?', (child,)).fetchone()[0]
                cursor.execute('INSERT INTO PhraseRelations (parent_id, child_id) VALUES (?, ?)', (parent_id, child_id))

    cursor.execute('SELECT COUNT(*) FROM Endings')
    if cursor.fetchone()[0] == 0:
        # Populate the Endings table
        cursor.executemany('INSERT OR IGNORE INTO Endings (ending) VALUES (?)', [(ending,) for ending in endings])
